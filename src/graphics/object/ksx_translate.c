...
/*   Updated: 2025/03/03 11:47:48 by ksorokol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ksx_get_wtm(t_matrix4 *p_m4, t_basis *p_basis , t_vector3 *p_o);
...
void	ksx_translate_obj(t_object *p_object)
{
	...
	ksx_get_wtm(&wtm, &p_object->basis, &p_object->basis.w_o);
...
static void	ksx_transform_obj_wr(t_object *p_object)
{
	...
	{
		p_object->pp_vrtx[idx]->wp = p_object->pp_vrtx[idx]->lp;
		// p_object->pp_vrtx[idx]->wp = ksx_vec3_add
		// 	(&p_object->pp_vrtx[idx]->lp, &p_object->basis.w_o);
		p_object->pp_vrtx[idx]->wnorm = p_object->pp_vrtx[idx]->lnorm;
		// p_object->pp_vrtx[idx]->wnorm = ksx_vec3_add
		// 	(&p_object->pp_vrtx[idx]->lnorm, &p_object->basis.w_o);
	}
	...
	while (pp_box && *pp_box)
	{
		idx = -1;
		while (++idx < 8)
			(*pp_box)->ver[idx].wp = (*pp_box)->ver[idx].lp;
			// (*pp_box)->ver[idx].wp = ksx_vec3_add
			// 	(&(*pp_box)->ver[idx].lp, &p_object->basis.w_o);
		pp_box++;
	}
	idx = -1;
	while (++idx < 4)
		p_object->axis[idx].wp = p_object->axis[idx].lp;
		// p_object->axis[idx].wp = ksx_vec3_add
		// 	(&p_object->axis[idx].lp, &p_object->basis.w_o);
}
...
static void	ksx_transform_obj_wt(t_object *p_object, t_matrix4 *p_wtm)
{
	...
	while (++idx < p_object->size_vrtx)
	{
		...
		ksx_transform(&p_object->pp_vrtx[idx]->wnorm,	// ??? may be need normalisation
	...
	idx = -1;
	while (++idx < 4)
		ksx_transform(&p_object->axis[idx].wp,
			p_wtm, &p_object->axis[idx].wp);
}
