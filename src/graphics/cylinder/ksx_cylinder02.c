...
/*   Updated: 2025/03/21 11:21:47 by ksorokol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			uint32_t size, uint32_t count)
{
	...
	while (idx[0] < count)
	{
		...
		while (idx[1] < size)
		{
			...
			pp_tri[idx[1]]->p_color = &p_object->color;
			pp_tri[idx[1] + size]->p_color = &p_object->color;
			idx[1]++;
		}
		idx[0]++;
	}
	...
	uint32_t size, t_vertex **pp_ver_)
{
	...
	while (idx[0] < size)
	{
		...
		pp_tri[idx[1]]->p_color = &p_object->color;
		pp_tri[idx[1] + size]->p_color = &p_object->color;
		pp_tri[idx[1] + size * 2]->p_color = &p_object->color;
		pp_tri[idx[1] + size * 3]->p_color = &p_object->color;
		idx[0]++;
	}
}
...
static void	ksx_init_cylinder_tri_cup(t_object *p_object, uint32_t size, t_vertex **pp_ver_b)
{
	...
	while (idx[0] < size)
	{
		...
		pp_tri[idx[0]]->p_color = &p_object->color;
		pp_tri[idx[0] + size]->p_color = &p_object->color;
		idx[0]++;
	}
}
